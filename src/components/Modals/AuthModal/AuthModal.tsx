    ...
    <div
      onClick={(e) => {
        if (e.target === e.currentTarget)
          setAuthModalState({ view: 'login', opened: false });
      }}
      className="fixed top-0 left-0 flex h-full w-full items-center justify-center bg-black/50"
    >
      <div className="relative h-full w-full bg-gray-50 px-5 pt-4 pb-8 shadow-header dark:bg-gray-700 md:h-auto md:w-[420px] md:rounded-xl md:px-[70px]">
        <button
          className="absolute top-4 right-5 rounded-md p-0.5 hover:bg-black/10"
          onClick={() => setAuthModalState({ view: 'login', opened: false })}
        >
